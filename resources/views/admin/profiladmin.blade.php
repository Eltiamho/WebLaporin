...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        @php
            $editId = session('edit_id') ?? old('id_admin');
            $deleteId = session('delete_id') ?? old('id_admin');
            $deleteNama = session('delete_nama') ?? '';
        @endphp
        ...
        @if (session('delete_error') && $deleteId)
            openDeleteModal('{{ $deleteId }}', '{{ $deleteNama }}');
        @endif
    });
</script>
