...
<body>
                ...
                <div class="rounded-lg shadow-md p-6 mb-6 border hover:shadow-lg transition-shadow duration-300 bg-white border-gray-200">
                    ...
                    @if ($isBencanaAlam)
                        <p class="mt-3 text-teal-700 font-semibold">
                            ...
                            @if ($row->total_donasi)
                                <span class="text-orange-600">Rp {{ number_format($row->total_donasi, 0, ',', '.') }}</span>
                            @else
                    ...
                    @if ($isActiveDonasi)
                        <div class="mt-4">
                            ...
                            @else
                                <a href="{{ url('/login') }}" class="bg-yellow-600 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded-lg transition-colors duration-200 btn-donasi">
                                    🔒 Silakan login untuk donasi
                                </a>
                            @endauth
    ...
    function tutupModal() {
        ...
        document.getElementById('lampiranContent').innerHTML = '';
    }
    </script>
